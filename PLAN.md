...
`content:previs` writes the blocked scenes — `output/previs/<show>/<episode>/scene_XX/blockout.mp4` plus `start.png` / `end.png` — and joins every scene into `output/previs/<show>/<episode>/blockout.mp4` so the clay cut plays as one episode. Each clay frame and its depth guide are one GPU draw of the location mesh (no image model; review this before spending GPU time on stills). Script marks stay on the empty floor. Previs stands each person on the open deck of that mesh, clear of landmark positions and raised mesh, and moves an interior camera until the lens is outside the stone and can see its look target. A camera already outside the mesh bounds stays where it was authored. A scene that is still missing leaves the episode file untouched. `content:frames` then writes:
...
`pnpm run content:plates` has Qwen-Image-Edit-2511 draw each location and stop. The pictures land in `output/plates/<show>/<locationId>/` so they can be reviewed before any mesh exists. `pnpm run content:assets` turns each reviewed plate into one mesh under `output/assets/<show>/<locationId>/`. It does not draw a plate. A missing plate stops the build. TRELLIS.2 turns the picture into one mesh. It does not search a catalog and it does not build a primitive stand-in. An unchanged location text, size, and polygon limit reuses `location.json`. A failed generation stops the build. ComfyUI's DecimateMesh node caps each mesh. The default polygon limit is 300,000. `pnpm run content:assets -- --triangles 300000` sets another limit, up to 50,000,000. The mesh is then scaled uniformly so it fits inside the location `sizeMeters` without changing the generated proportions. ComfyUI must already be running. `pnpm run content:landmarks` runs after that mesh exists. It shades the mesh from known cameras, asks Florence-2 where each landmark name is, and writes `position` into `shows/<id>/script.json`. Review images land in `output/landmarks/<show>/<locationId>/`. A position that is already set is kept unless `--force` is passed. `content:render` does not run this step. `pnpm run content:asset-deps` installs trimesh, moderngl, transformers, timm, and einops into the ComfyUI Python. trimesh is what glTF and OBJ files need. moderngl is what clay previs draws with. transformers, timm, and einops load Florence-2.
...
A landmark is an optional position in the generated location mesh: `[X right, Y forward, Z up]` in meters. Leave it empty until the mesh exists. `pnpm run content:landmarks` writes that position after the mesh exists. A position already set is kept unless that command is run with `--force`. A `need` search object, a catalog `assetId`, and a primitive `kind` are rejected. Show-level `props` declares ids used by `propTracks` (`appearance`, optional `sizeMeters`). Character `proxy` (`heightMeters`, `build`) is optional; omitted proxies use a 1.72m average mannequin. Clay mannequins are still the people. Each location is one generated mesh of that place.
...
- `content:previs` is the blocked scene: a clay playblast of the camera and bodies, plus start and end frames. No image or video model runs. People stand on the open deck of the location mesh, and an interior camera is moved out of solid mesh; exterior cameras outside the mesh bounds stay as authored. Qwen restyles those frames. The end still is its own blockout, not an edit of the start photograph, because Qwen-Image-Edit keeps the photograph's camera. Empty `characterIds` are environments and skip the face pass. The first two `characterIds` are painted onto the matching face masks; additional people stay as the bodies in the blockout. Dialogue may be a group; `speakerId` names who talks and must be in `characterIds`.

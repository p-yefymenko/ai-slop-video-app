...
A landmark is an optional position in the generated location mesh: `[X right, Y forward, Z up]` in meters. Leave it empty until the mesh exists. The plate is not a calibrated floor plan, and the mesh does not name its parts, so that coordinate is filled in by hand. A `need` search object, a catalog `assetId`, and a primitive `kind` are rejected. Show-level `props` declares ids used by `propTracks` (`appearance`, optional `sizeMeters`). Character `proxy` (`heightMeters`, `build`) is optional; omitted proxies use a 1.72m average mannequin. Clay mannequins are still the people. Each location is one generated mesh of that place.
...
- Generated files are skipped when present. After a structural script rewrite, use `pnpm run content:archive -- <show-id>` before generating fresh frames. It archives that show's plates, meshes, previs, stills, and clips, and keeps the reviewed character portraits in `output/frames/<show>/characters/`.

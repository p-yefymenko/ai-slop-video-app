...
7. [ ] **Install ComfyUI + LTX locally on the GPU machine** — run `pnpm run content:setup-comfy` once (clones ComfyUI and the LTX/GGUF/VHS custom nodes into `content-pipeline/.comfyui` and installs CUDA PyTorch). If you already ran setup before CUDA torch existed, run `pnpm run content:comfy-torch`. Then `pnpm run content:models` (downloads the ~13GB Q4_K_M GGUF, the ~2.4GB video VAE, and a tiny official-checkpoint metadata stub for the Gemma API node). Leave `pnpm run content:comfy` running so the API is at `http://127.0.0.1:8188`. Open that URL, Load `ltx_gemma_api.json`, and fix any missing-node / missing-file errors before generating.
...
- **Content generation (offline, not part of the live app):** ComfyUI + LTX (Q4_K_M GGUF quantization) running locally on the dev machine, with Gemma API used for text-encoder conditioning to stay within 16GB VRAM. Output MP4s are uploaded to R2 via a script, not generated at runtime.
...
3. `pnpm run content:models` — downloads `ltx-2-19b-distilled_Q4_K_M.gguf` (~13GB) into `content-pipeline/.comfyui/models/diffusion_models/`, the matching video VAE into `models/vae/`, and a tiny official LTX-2.3 distilled-1.1 safetensors stub into `models/checkpoints/` so `GemmaAPITextEncode` can read a `model_id` the LTX prompt-embedding API actually serves, without downloading the 46GB checkpoint. This is the quantization that fits 16GB VRAM (RTX 5070 Ti) when text encoding is offloaded to the LTX API.

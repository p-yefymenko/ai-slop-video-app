...
9. [ ] Run `pnpm run content:frames`. Review `characters/` then `scene_*_start.png` in `content-pipeline/output/frames/<show>/`. Keep, replace, or delete any file you do not like. Then run `pnpm run content:generate` and `pnpm run content:upload` on your own machine (the RTX 5070 Ti). ComfyUI must already be running from the previous step.
...
```json
{
  "scripts": {
    ...
    "content:plates": "node scripts/run-python.cjs content-pipeline/scripts/build_assets.py --stage plates",
    "content:assets": "node scripts/run-python.cjs content-pipeline/scripts/build_assets.py --stage meshes",
    "content:landmarks": "node scripts/run-python.cjs content-pipeline/scripts/label_landmarks.py",
    ...
    "view": "node scripts/view-show.cjs",
...
- **Content generation (offline, not part of the live app):** Each episode first defines a deterministic 3D blocking timeline: measured sets, character/prop keyframes, and physical cameras. `content:previs` renders that timeline into a clay playblast and start/end frames with no image or video model. **Qwen-Image-Edit-2511** with its 4-step Lightning LoRA (CFG 1, AuraFlow shift 3.1) then generates each still from that previs. Picture 1 is the depth map. Pictures 2 and 3 are the geometric edges and the normal map. A shot with people keeps that character's generated mesh in the depth, so their features, position, and occlusion stay. The OpenPose skeleton is the next picture and sets the joints. A character with no mesh yet falls back to a volume that is left out of the edges. The sampler starts from noise at denoise 1.0, so the shaded clay frame is not the picture being edited. Identity faces are a second masked pass. Character portraits use the same Lightning settings. Every shot is then animated with **LTX-2.3 distilled-1.1** from that still (and an end guide when blocking or camera actually change). Dialogue additionally uses audio-video modality guidance and explicit lip-sync conditioning. Gemma API supplies text conditioning within 16GB VRAM.
...
`content:previs` writes the blocked scenes — `output/previs/<show>/<episode>/scene_XX/blockout.mp4` plus `start.png` / `end.png` — and joins every scene into `output/previs/<show>/<episode>/blockout.mp4` so the clay cut plays as one episode. Each clay frame and its depth guide are one GPU draw (no image model; review this before spending GPU time on stills). A location with no people is one mesh of the place, and the camera stays where it was authored. A location with people is an open floor plus one mesh per landmark, placed at the script position and fitted to that landmark's size. People and cameras stay on the marks in the script. A scene that is still missing leaves the episode file untouched. `content:frames` then writes:

1. Character stills — `output/frames/<show>/characters/<id>.png` (human review, then the masked face pass)
2. Scene stills — `output/frames/<show>/<episode>/scene_XX_start.png`, plus `scene_XX_end.png` when a generative scene's blocking actually changes

`content:generate` writes `output/generate/<show>/<episode>/scene_XX.mp4` and concatenates `episode.mp4` in that same folder. LTX only sees the reviewed scene still, never the character portrait.

`pnpm run content:plates` has Qwen-Image-Edit-2511 draw and stop. An empty location is one picture of the place in `output/plates/<show>/<locationId>/`, prompted as a movie set. A location with people is one picture per landmark in `output/plates/<show>/<locationId>/<landmarkId>/`, prompted as one isolated object. Landmarks with the same appearance and size are drawn and meshed once. Each character is one full-body plate in `output/plates/<show>/characters/<id>/`, then one mesh fitted to their standing height. `pnpm run content:assets` turns each reviewed picture into one mesh and does not draw a plate. A missing plate stops the build. TRELLIS.2 turns the picture into one mesh. It does not search a catalog and it does not build a primitive stand-in. An unchanged text, size, and polygon limit reuses the mesh. A failed generation stops the build. ComfyUI's DecimateMesh node caps each mesh. The default polygon limit is 300,000. `pnpm run content:assets -- --triangles 300000` sets another limit, up to 50,000,000. The mesh is then scaled uniformly into the requested box without changing the generated proportions. An empty location uses `sizeMeters`. A landmark uses its own `size`. ComfyUI must already be running. `pnpm run content:landmarks` runs after an empty location's mesh exists. It shades that mesh from known cameras, asks Florence-2 where each landmark name is, and writes `position` into `shows/<id>/script.json`. It skips a location that has people, because those positions are already in the script. Review images land in `output/landmarks/<show>/<locationId>/`. A position that is already set is kept unless `--force` is passed. `content:render` does not run this step. `pnpm run content:asset-deps` installs trimesh, moderngl, transformers, timm, and einops into the ComfyUI Python. trimesh is what glTF and OBJ files need. moderngl is what clay previs draws with. transformers, timm, and einops load Florence-2.

`content:render` runs `content:assets`, `content:previs`, `content:frames`, and `content:generate` in that order and forwards the same selection arguments to all four stages. It does not draw plates. Run `content:plates` and review those images before `content:render`. `pnpm run view -- --show <show-id>` opens that show at `http://127.0.0.1:5174`. It reads location meshes and scene descriptions as glTF Y-up. Open VSX has file-level GLB previews (`slevesque.vscode-3dviewer`, and the OHZI GLTF/GLB Viewer); the stage viewer does not depend on them.

Authored files stay in `shows/<id>/script.json`. Everything generated from the script stays in `output/<command>/<show>/`. Schema space is Z-up and +Y forward; glTF is Y-up. `content-pipeline/scripts/coords.py` is the only module that converts between them. `pnpm run content:assets -- --credits` rewrites `docs/CREDITS.md` from the location records under `output/assets/`.
...
7. Then `pnpm run content:frames`. Review `content-pipeline/output/frames/<show>/characters/` and each episode’s `scene_*_start.png`. Then `pnpm run content:generate`. `pnpm run content:render` runs assets, previs, frames, and video in sequence with no still-review pause.
...
One JSON file per show: `content-pipeline/shows/<id>/script.json`. Shape is `ShowScript` in `packages/shared/src/script.ts`. The show folder name must match `id`. `pnpm run content:validate` checks every show file and prints a path plus a reason for each problem. A file left at `scripts_input/<id>.json` is still read when `shows/<id>/script.json` is absent. `pnpm run content:migrate-output` prints the move from the old show folders into `output/plates`, `output/assets`, `output/previs`, `output/frames`, and `output/generate`. It changes nothing until `--apply`.

A landmark on a location with no people is an optional position in that location's mesh: `[X right, Y forward, Z up]` in meters. Leave it empty until the mesh exists. `pnpm run content:landmarks` writes that position. A position already set is kept unless that command is run with `--force`. A landmark on a location with people is one object: `position`, `size`, and `appearance`. `content:assets` fits its mesh into that size and previs places it at that position. A `need` search object, a catalog `assetId`, and a primitive `kind` are rejected. Show-level `props` declares ids used by `propTracks` (`appearance`, optional `sizeMeters`). Character `proxy` (`heightMeters`, `build`) is optional; omitted proxies use 1.72m. `content:plates` and `content:assets` build each character the same way as a landmark: one full-body plate from `promptBlock`, then one mesh fitted uniformly to their standing height. Previs stands that mesh on the character's mark and turns it with `bodyYawDegrees`. A missing mesh falls back to a volume. An empty location is one generated mesh of that place. A location with people is the open floor plus those landmark meshes.

Show JSON contains only show-specific authoring data: identity descriptions, measured locations,
episode spatial timelines, and scenes. Shared Qwen/LTX templates live once in
...
- Each scene camera is a keyframe path (`position`, `lookAt`, `verticalFovDegrees`, optional `rollDegrees`) on the episode clock. One keyframe holds the camera still; more than one interpolates. Heads may leave the frame (OTS, inserts, ECU).
- Scene duration is `timeRangeSeconds`. Do not store a parallel `durationSeconds`. End guides are derived: scenes whose timeline or camera actually change get one; static scenes do not.
- `content:previs` is the blocked scene: a clay playblast of the camera and bodies, plus start and end frames. No image or video model runs. People and cameras stay on the script marks. An empty location draws its one mesh. A location with people draws the floor and each landmark mesh at its script position. Qwen generates each still from the depth, edges, and normals of that frame. A shot with people keeps that character's generated mesh in the depth, so their features, position, and occlusion stay. The OpenPose skeleton is the next picture and sets the joints. A character with no mesh yet falls back to a volume that is left out of the edges. The shaded clay picture is not the edit source. The end still is generated from the end guides, because Qwen-Image-Edit keeps the camera of whatever picture it is given. Empty `characterIds` are environments and skip the face pass. The first two `characterIds` are painted onto the matching face masks; additional people stay as the bodies in the blockout. Dialogue may be a group; `speakerId` names who talks and must be in `characterIds`.
- `spatialTimeline` and each scene camera define eyelines; no parallel prose screen-direction field exists.
- `speakerId` turns on LTX MultimodalGuider for lip-sync. Omit on silent scenes.
- `imagePrompt` is optional wardrobe, expression, and atmosphere. Identity is the PNG. Geometry comes from the previs depth, edges, and normals, and from the pose when people are in the shot. Omit it when the location block and those guides are enough.
- `imagePrompt` may name only characters in `characterIds`.
- `videoPrompt` is the spoken line and non-spatial performance. Camera and blocking come from the timeline and start/end frames. Omit it on silent scenes.
- The Python loader validates only render-critical structure such as required fields, known location/visible-character IDs, sequential output numbers, and positive duration. It does not reject scripts for creative guidance such as pacing, dialogue length, scene semantics, or prompt wording.
- Generated files are skipped when present. After a structural script rewrite, use `pnpm run content:archive -- <show-id>` before generating fresh frames. It archives that show's plates, meshes, landmark views, previs, stills, and clips, and keeps the reviewed character portraits in `output/frames/<show>/characters/`.
- Character portraits, scene stills, and clips use stable per-scene seeds by default, so an unchanged scene reproduces instead of changing randomly between full renders.
- Iterate on one scene with `pnpm run content:frame -- --show <show-id> --episode <n> --scene <n> --force`, then test only its video with the same filters through `pnpm run content:clip`. To intentionally reroll a bad still or clip, add `--seed <0-4294967295>` to that selected `--force` command. A partial clip render never overwrites `episode.mp4`.

...
- `content:previs` is the blocked scene: a clay playblast of the camera and bodies, plus start and end frames. No image or video model runs. People and cameras stay on the script marks. An empty location draws its one mesh. A location with people draws the floor and each landmark mesh at its script position. Qwen generates each still from the depth, edges, and normals of that frame. A shot with people uses their pose in place of the normals. The shaded clay picture is not the edit source. The end still is generated from the end guides, because Qwen-Image-Edit keeps the camera of whatever picture it is given. Empty `characterIds` are environments and skip the face pass. The first two `characterIds` are painted onto the matching face masks; additional people stay as the bodies in the blockout. Dialogue may be a group; `speakerId` names who talks and must be in `characterIds`.

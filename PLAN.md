...
- **Content generation (offline, not part of the live app):** Each episode first defines a deterministic 3D blocking timeline: measured sets, character/prop keyframes, and physical cameras. `content:previs` projects that state into a proxy sketch, a depth map, an OpenPose skeleton, and a contact sheet. **Qwen-Image-Edit-2511** paints each still in two passes: a 20-step structure pass locked to the depth and pose images by the InstantX Union ControlNet, then an img2img refine at denoise 0.4. The structure pass sees identity portraits only. The refine pass also gets the proxy sketch as loose context. The proxy is not the geometry constraint. Character portraits stay on the 4-step Lightning LoRA. Every shot is then animated with **LTX-2.3 distilled-1.1** from that still (and an end guide when blocking or camera actually change). Dialogue additionally uses audio-video modality guidance and explicit lip-sync conditioning. Gemma API supplies text conditioning within 16GB VRAM.
...
- Spatial stills use identity references plus that shot's previs. The start still uses the start depth, pose, and proxy; the end still uses the end set. The end still is not an edit of the start photograph, because Qwen-Image-Edit keeps the photograph's camera. Depth and OpenPose go through the InstantX Union ControlNet. The proxy sketch is only a loose picture reference on the refine pass. Empty `characterIds` are environments and skip pose control. The first two `characterIds` are Qwen identity Pictures; additional people exist in the depth and pose maps. Dialogue may be a group; `speakerId` names who talks and must be in `characterIds`.
